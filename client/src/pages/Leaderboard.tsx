...
const StudyHours: React.FC = () => {
  ...
  const handleLog = (e: React.FormEvent) => {
    ...
    if (!isNaN(h) && h > 0) {
      ...
      localStorage.setItem('study_streak', String(lastLogDate === today ? streak : streak + 1));
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-blue-100 via-fuchsia-100 to-pink-100">
      <div className="bg-white/90 rounded-3xl shadow-2xl border-4 border-blue-200 max-w-lg w-full flex flex-col gap-6 p-10 items-center">
        ...
        <div className="text-2xl font-bold text-fuchsia-500 mb-2">Your Total: {hours} hours</div>
        <div className="text-lg font-semibold text-blue-700 mb-4">Current Streak: {streak} {streak === 1 ? 'day' : 'days'}</div>
          ...
          <tbody>
            {leaderboardData.map((user, idx) => (
              <tr key={user.name} className={idx === 0 ? 'bg-fuchsia-100 font-bold' : 'bg-blue-50'}>
                ...
                <td className="px-3 py-2 rounded-r-xl">{user.name === 'You' ? streak : user.streak}</td>
              </tr>
            ))}
          </tbody>
        </table>
