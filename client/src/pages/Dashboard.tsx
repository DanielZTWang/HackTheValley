      ...
      <div className="bg-white/90 rounded-3xl shadow-2xl border-4 border-fuchsia-200 max-w-2xl w-full flex flex-col gap-8 p-10 items-center">
        ...
        <div className="w-full flex flex-col md:flex-row gap-8 justify-between">
          <div className="flex-1 flex flex-col items-center gap-4">
            ...
            <div className="text-5xl font-extrabold text-fuchsia-400">{hours}</div>
            <form className="flex gap-2 mt-2" onSubmit={handleLog}>
              <input
                className="border border-fuchsia-200 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-fuchsia-200 w-24 text-center"
                type="number"
                min="0"
                step="0.1"
            ...
            <div className="mt-2 text-fuchsia-600 text-lg font-semibold text-center">{message}</div>
          </div>
          <div className="flex-1 flex flex-col items-center gap-4">
            <div className="text-2xl font-bold text-purple-500 mb-2">Your Friends</div>
            <ul className="w-full flex flex-col gap-2">
              {friends.map(friend => (
                <li key={friend.name} className="flex justify-between items-center bg-purple-50 rounded-xl px-4 py-2 shadow">
                  <span className="font-semibold text-purple-700">{friend.name}</span>
                  <span className="text-fuchsia-400 font-bold">{friend.hours} hrs</span>
                </li>
              ))}
            </ul>
          </div>
